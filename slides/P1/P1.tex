...
\begin{frame}
    ...
    \only<2>{
        \begin{tikzpicture}[overlay]
            \node[anchor = base, text=red!75!black, text width = 13cm, align = center] at ([xshift=-1cm,yshift=-3cm]current page.center){\LARGE This is not a Computer Science Class!};
        \end{tikzpicture}
    }
\end{frame}
...
\begin{frame}
    ...
    \only<1>{
        \setbeamercolor{block title}{bg=red!75!black}%bg=background, fg= foreground
    ...
    \only<2,3>{
        ...
        \begin{tikzpicture}
            ...
            \only<3>{
                \draw[red!75!black, dashed] (9,0) circle (1.4cm);
            }
